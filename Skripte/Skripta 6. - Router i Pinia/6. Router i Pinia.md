...
**Posljednje ažurirano:** 3. svibnja 2025.
...
Do sad nismo koristili *url linkove* za navigaciju po stranici unutar intrnetskog preglednika. Za to postoji službena *Vue.js* paket **Vue Router** koja omogućuje dinamičko upravljanje navigacijom, ugniježđenim rutama, uz podršku za povijest, tranzicije i naprednu kontrolu putanje s modularnom konfiguracijom.
...
1. **Slučaj**: ako već imamo postojeći projekt onda moramo **intalirati** paket:
...
Kada nammjestimo početnu router logiku, ona nam može izgledati nešto ovako:
...
Funkcija `createRouter()` prima objekt kao argument koji sadrži opcije za izradu router-a. Nama su zasad bitne samo dvije opcije:
...
Međutim sada kada imamo taj **parametar** možemo mu i pristupiti puemt `$route.params.username` sintakse.
...
Kod izrade **dinamičkih ruta** može se koristiti regex za definiranje ugniježđenih parametara ili definiranje svojstva parametra. Ako se prisjetimo problema `createWebHistory()` funckije, tu znamo da kod nepostojeći ruta dobivamo *404 grešku* (*npr. /apple*).
...
- Sada ako odemo na rutu `/home` neće nas preusmjerit na rutu `/` ali će router znati da je to trenutno aktivna ruta.
...
Vue router paket dolazi sa dva ugrađena elementa: `<RouterLink>` i `<RouterView/>`. Dok nam **RouterView** služi za prikaz samih ruta, **RouterLink** se ponaša kao običan `<a/>` element gdje umjesto atribut *href* koristi atribut **to**.

```html
  <RouterLink to="/users/ivan">User</RouterLink>
```

S obzirom da koristimo Vue, znači da može atribut to vezati ko i sve druge atribute `v-on` direktivom `:` skraćeno.
...
**Router** cijelo vrijeme prati na kojoj se ruti nalazimo, što mu omogućuje da zna koji je trenutni aktivni link to jest koji `<RouterLink>` element je trenutno aktivan. 

`RouterLink` se smatra aktivnim ako:

- Odgovara istom zapisu rute (tj. konfiguriranoj ruti) kao i trenutna lokacija.
- Ima iste vrijednosti parametara kao i trenutna lokacija.
...
Ako je trenutna lokacija puta `/users/ivan/avatar`, tada se obje rute smatraju aktivnim, pa im se primijenjuje klasa `router-link-active`. No, samo je druga ruta točno aktivna (*exact*), pa bo joj se primijenila klasa `router-link-exact-active`.

![alt text](Slike/routerlink.png)

#### Router funkcije

Kada želimo mjenjati rute putem funckija iz drugih djelova aplikacije bez korištenja elementa `RouterLink` u tom slučaju koristimo **router funkcije**.
...
* **from** – objekt koji predstavlja trenutnu rutu iz koje korisnik dolazi (*polazna ruta*)
...
U ovom zadatku ćete izraditi Vue router na temelju prošlog *samostalnog zadatka za vježbu 4.* iz mape `Primjeri/primjer_vue_zadatak` ili preuzmite zip datoteku projekta s Merlina.

Nadogradite aplikaciju tako da koristi Vue router za navigaciju između stranica.
...
- *Stranice*  
  ...
  - `PageGrafika.vue` – Stranica o izradi grafike  
    - URL: `/grafika`
    - Name: `grafika`
  - `NotFound.vue` - Stranica ako url ne postoji, **potrebno izraditi i dodati stranicu**

> Dodajte *blur in/out* tranzicije između stranica.
...
*Pa čemu onda Pinia ako već postoji ugrađena funkcionalonst Composables koja radi istu stvar?*

Jedina velika razlika je da se Pinia ukomponira u **Vue Devtools** što omogućuje u uvid stanje aplikacije to jest možemo promatrati i mjenjati stanje aplikacije.
...
Postoje i slučajevi kada je Pinia korisna i za podatke koji su razasuti po različitim razinama hijerarhije unutar jednog prikaza. To je prilično uobičajeno kod složenijih prikaza s više razina ugnježđivanja, gdje je nepraktično i neuredno prosljeđivati podatke s viših razina na niže. Ako dođemo do te točke, vjerojatno je bolje samo napraviti Pinia store i pristupati podacima direktno iz tih komponenti. Na taj način također izbjegavamo potrebu za proslijeđivanjem događaja prema gore kroz više razina – – to jest tzv. *bubble up events* - što je još gore od samog prosljeđivanja podataka. Jednostavno izmijenimo podatke iz duboko ugniježđene komponente direktno.
...
const pinia = createPinia() // izrada pinie
const app = createApp(App)

app.use(pinia) // // korištenje pinie
app.mount('#app')
```
...
Onda možemo unutar te datoteke definirati **spremnik** pomoću funkcije `defineStore()` koja zapravo i stvara novi **spremink**:
...
S obzirom da cijelo vrijeme koristimo Composition API, tako ćemo ga dalje nastaviti koristiti u Piniji. 
