...
> Ako korisnik nije prijavljen ne može pristupiti stranici `UserView.vue`
...
**Pri registraciji** na `SignUpView.vue` stranici:
  - treba provjeriti **validnost** lozinke:
    - barem jedno *veliko*, *malo slovo*, *broj* i *specijalni znak* (`!"#$%&/()=?*+-`)
    - da sadrži barem 8 znakova
  - treba provjeriti **validnost** emaila:
    - da sadrži `@` simbol
    - da sadrži `.` simbol nakon `@` simbola
  - treba provjeriti **validnost** usernamea:
    - da sadrži između *4* i *16* znakova
    - da se sastoji samo od *alfanumeričkih* znakova
