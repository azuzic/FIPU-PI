...
- [[1] Vue.js osnove](#1-vuejs-osnove)
    ...
    - [Uvod](#uvod)
        - [Single Page Application SPA](#single-page-application-spa)
            - [Primjer jednostavne vounter aplikacije](#primjer-jednostavne-vounter-aplikacije)
...
- [Postavljanje aplikacije/projekta](#postavljanje-aplikacijeprojekta)
    ...
    - [Struktura projekta](#struktura-projekta)
        ...
        - [main.css](#maincss)
- [Tailwind](#tailwind)
    - [Prednosti Tailwind-a](#prednosti-tailwind-a)
    ...
    - [Instalacija Tailwind-a](#instalacija-tailwind-a)
...
Tailwind CSS omogućuje jednostavno korištenje **linearnih gradijenata** putem ugrađenih klasa. Gradijenti se definiraju pomoću 2 osnovna svojstva:  

1. **Smjer gradijenta** – određuje smjer gradijenta  
2. **Početna, prijelazna (opcionalno) i završna boja** – definiraju boje gradijenta

*Smjer gradijenta određuje se pomoću `bg-gradient-to-<smjer>`, gdje `<smjer>` može biti:*
...
Boje gradijenta se definiraju pomoću `from-<boja>` za početnu boju i `to-<boja>` za završnu boju. Opcionalno, može se koristiti `via-<boja>` za srednju boju.
...
*`Width`, `height` i `size` djele iste klase:*
...
Tailwind CSS omogućuje jednostavno postavljanje **zaobljenih rubova** (`border-radius`) pomoću klasa `rounded-<vrijednost>`.  
...
Prilagođavanje prikaza slika vrši se pomoću `object-fit` svojstava. Ove klase određuju kako se sadržaj slike prilagođava unutar okvira.  
...
Tailwind omogućuje definiranje **interaktivnih stanja**, **responzivnih klasa** i **tematskih prilagodbi** pomoću **prefiksa**.  
...
| `active:`   | Aktivira se kada je element trenutno pritisnut (kliknut)             |
...
Tailwind omogućuje jednostavno definiranje stilova za tamni način pomoću `dark:` prefiksa. Da biste aktivirali tamni stil, dodajte klasu `dark` na najviši DOM element. Klasa `dark` može se dinamički dodavati i uklanjati za prebacivanje između tamne i svijetle teme.  
