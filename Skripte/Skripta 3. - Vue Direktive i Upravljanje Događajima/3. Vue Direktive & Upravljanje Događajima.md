...
| `v-model`   | Povezuje varijable sa input poljima za dvosmjerno vezivanje podataka      |
...
Direktiva `v-for` koristi se za prikaz popisa elemenata na temelju niza. Sintaksa je `item in items`, gdje `items` predstavlja podatke, a `item` je trenutna stavka u iteraciji.  
...
*Unutar `v-for` izraza moguće je koristiti i **indeks** trenutne stavke.  koristeći sintaksu `(item, index) in items`:*
...
- Unutar svake kategorije, pomoću ugniježđene `v-for` petlje, iteriramo kroz proizvode i prikazujemo ih

#### Modificiranje polja, filtriranje/sortiranje

U Vue.js možemo dinamički mijenjati polja pomoću metoda poput **sort()**, **filter()** i **map()**, te ih koristiti unutar `v-for`.
...
- *v-pre*: neće procesirat sadržaj

## **Upravljanje događajima**

U Vue.js, događaji se mogu koristiti za interakciju sa korisnicima i upravljanje aplikacijom. Vue nudi jednostavan način za preslušavanje i rukovanje događajima putem direktiva `v-on`.
...
```
- Metoda može biti funckija ili JavaScript kôd.
...
Možemo koristiti modifikatore događaja za dodavanje specifičnih ponašanja, kao što su spriječavanje osnovnih akcija `event.preventDefault()`.
...
- Ako nema nijednog proizvoda u kođarici, treba pisati *"Košarica je prazna!"*.
